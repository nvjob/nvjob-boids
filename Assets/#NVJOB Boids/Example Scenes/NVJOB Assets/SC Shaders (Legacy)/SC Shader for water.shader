...
Shader "#NVJOB/SC Shaders (Legacy)/For Water" {
...
_SpeedOcclusion("Speed Occlusion", float) = 1.0
...
half _Shininess, _BiasNormal, _IntensityNm, _OcclusionMapUv, _IntensityOc, _IntensityRef, _Saturation, _Contrast, _Brightness, _SpeedOcclusion;
samplerCUBE _Cube;
...
half2 uvodirA = IN.uv_MainTex * _OcclusionMapUv;
half2 uvodirB = IN.uv_MainTex * _OcclusionMapUv;
uvodirA.x += _Time.y * 0.1 * _SpeedOcclusion;
uvodirB.y += _Time.y * 0.13 * _SpeedOcclusion;
half oc = tex2D(_OcclusionMap, uvodirA).r;
half ocd = tex2D(_OcclusionMap, uvodirB).r;
half ocf = (oc + ocd) * 0.5;
ocf *= _IntensityOc;
...
o.Albedo = ((color - 0.5) * _Contrast + 0.5) * ocf;
o.Gloss = tex.a;
